...
object streaming {
  ...
  def streamFromLists[A](xxs: List[A]*): Stream[Pure, A] =
    Stream.emits(xxs).flatMap(streamFromList)
  ...
  def periodically(period: FiniteDuration)(
    ...
    Stream.emit(()) ++ S.fixedDelay[IO](period)
  ...
  def retryOp[A](op: IO[A], tries: Int, backOff: FiniteDuration)(
    ...
    periodically(backOff)
      .evalMap(_ => op.attempt)
      .take(tries)
  ...
  def periodicallyRetry[A](
    ...
    periodically(period) >>
    Stream.eval(signalOf[IO, Boolean](false)).flatMap { sig =>
      retryOp(op, tries, backOff)
        .attempt
        .filter(_.isRight)
        .rethrow
        .interruptWhen(sig)
        .concurrently(
          Stream.eval(Timer[IO].sleep(period) >> sig.set(true))
        )
    }
}
