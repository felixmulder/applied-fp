...
object traverse {
  ...
  def allOrErrors[E <: Throwable, A](xs: List[Either[E, A]]): ValidatedNel[Throwable, List[A]] =
    xs.map(_.toValidatedNel).sequence
